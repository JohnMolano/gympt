import 'package:flutter/material.dart';

class ColorConstants {
  static const textColor = Color.fromARGB(255, 0, 0, 0);
  static const primaryColor = Color.fromARGB(255, 226, 75, 15);

  static const textBlack = Color(0xFF1F2022);
  static const white = Color(0xFFFFFFFF);
  static const grey = Color(0xFFB6BDC6);
  static const loadingBlack = Color(0x80000000);

  static const textFieldBackground = Color(0xFFFBFCFF);
  static const textFieldBorder = Color(0xFFB9BBC5);

  static const disabledColor = Color(0xFFE1E1E5);
  static const errorColor = Color.fromARGB(255, 107, 8, 8);

  static const homeBackgroundColor = Color(0xFFFBFCFF);
  static const textGrey = Color(0xFF8F98A3);

  static const cardioColor = Color.fromARGB(255, 226, 75, 15);
  static const armsColor = Color.fromARGB(255, 37, 37, 37);
}
